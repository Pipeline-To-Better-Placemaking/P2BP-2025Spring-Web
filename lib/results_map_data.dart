...
class VisualizedResults {
  ...
  //List<Map<String, dynamic>> fetchedPolylines = await loadPathsFromFirebaseIdentifyingAccessTests(testID);
    ...
    return VisualizedResults(
      ...
      polylines: _parsePolylines(fetchedPolylines, collectionName),
  ...
  static Set<Polyline> _parsePolylines(List<dynamic> polylinesData, String collectionName) {
    return polylinesData.map((polyline) {
      ...
      return Polyline(
        ...
        color: Colors.red, // Set a color for the polyline
    ...
    if (polygonData.isNotEmpty) {
      for (var item in polygonData) {
        ...
        if (item is List<LatLng>) {
          ...
          else if (collectionName == "nature_prevalence_tests" && polygonColor == "waterBody") 
          {
            strokeColor = Color.fromRGBO(0, 0, 70, 1); // Dark blue
            fillColor = Color.fromRGBO(0, 0, 255, 0.2); // Light blue with 20% opacity
          }
          else if (collectionName == "identifying_access_tests" && polygonColor == "black")
          {
